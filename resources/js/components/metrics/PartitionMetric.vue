<template>
    <div class="min-h-[15rem] p-4 ">
        ...
        <div class="flex flex-wrap sm:flex-nowrap gap-9 justify-evenly">
            ...
            <div class="relative sm:h-40 sm:w-40 h-24 w-24 ">
                <canvas :id="chartID" />
            </div>
        </div>
    </div>
...
onMounted(() => {
    ...
    watch(
        () => props.uiPath, (newP, oldP) => loadMetric()
    );
})
...
const loadMetric = function() {
    endpoint.query({ endpoint: 'value', params: props.params, uiPath: props.uiPath })
        .then(response => {
            ...
            if (!response.partition || response.partition.length == 0) {
                chart.data = {
                    labels: [],
                    datasets: []
                };
...
const filter = function(item) {
    console.log(chartData.value, props.metric, item)
